...
A beginner-friendly guide to the concepts you need to understand the Custodian Testnet Faucet Tool.
...
In this tool, every asset has a **Custodian asset ID** like `HTETH` (Holesky testnet ETH) or `HTETH:GOUSD` (the GOUSD token on Holesky Ethereum). The colon format `CHAIN:TOKEN` tells you the token lives on that chain.
...
| **Asset ID** | Custodian's identifier for an asset (e.g., `HTETH`, `TSOL`) |
