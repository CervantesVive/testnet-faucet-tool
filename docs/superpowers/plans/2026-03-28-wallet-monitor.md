...
**Architecture:** Two new `core/` modules (`alerting.py`, `monitor.py`) keep alert dispatch and check-and-act logic separate. Two new CLI commands (`check` one-shot, `monitor` daemon) share the same underlying `run_check()` function. All data lives in `~/.testnet-faucet/` (renamed from `~/.bitgo-faucet/`).
...
Change line 6 from:
```python
DB_PATH = Path(os.environ.get("BITGO_FAUCET_DB_PATH", Path.home() / ".bitgo-faucet" / "rate_limits.db"))
```
To:
```python
...
git commit -m "refactor: rename data dir from .bitgo-faucet to .testnet-faucet"
```
