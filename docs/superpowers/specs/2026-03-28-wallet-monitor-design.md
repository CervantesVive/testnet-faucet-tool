...
All data files move from `~/.bitgo-faucet/` to `~/.testnet-faucet/`. The env var `BITGO_FAUCET_DB_PATH` is renamed to `FAUCET_DB_PATH`. Existing files in the old location are not migrated automatically — the new path is used on first run.
...
| `core/monitor.py` | New |
| `core/rate_limiter.py` | Rename `~/.bitgo-faucet/` → `~/.testnet-faucet/`, `BITGO_FAUCET_DB_PATH` → `FAUCET_DB_PATH` |
| `core/logger.py` | Rename `~/.bitgo-faucet/` → `~/.testnet-faucet/` |
