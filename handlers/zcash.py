...
class ZcashHandler(BaseHandler):
    """Handler for Zcash (TBD — rpc_url not yet configured)."""
    ...
    def validate_address(self, address: str) -> bool:
        if not address:
            return False
        return bool(re.match(r'^tm[a-zA-Z0-9]{32,}$', address))

    async def get_faucet_balance(self) -> dict[str, str]:
        asset_id = self.config.get("blockchain", "Zcash")
        rpc_url = self.config.get("rpc_url", "TBD")
        if rpc_url == "TBD":
    ...
    def supported_assets(self) -> list[str]:
        return [k for k, v in load_registry().items() if v.get("family") == "zcash"]
